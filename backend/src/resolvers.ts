...
export const resolvers: Resolvers = {
    ...
    Comment: {
        author: async (parent, _, context: DataSourceContext) => {
            const user = await context.dataSources.db.user.findUnique({
                where: {id: parent.authorId},
            });

            if (!user) {
                throw new GraphQLError("Auteur du commentaire non trouvé");
            }
        ...
        post: async (parent, _, context: DataSourceContext) => {
            const post = await context.dataSources.db.post.findUnique({
                where: {id: parent.postId},
            });

            if (!post) {
                throw new GraphQLError("Article du commentaire non trouvé");
            }
